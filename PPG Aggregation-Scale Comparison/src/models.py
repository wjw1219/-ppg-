...
class AblationModel(nn.Module):
    def __init__(self, mode, clinical_group_dims, ppg_dim=13, hidden_dim=32, heads=2, dropout=0.25, weeks=26):
        super().__init__()
        self.mode = mode
        ...
        if mode in {"ppg", "fusion"}:
            self.ppg_encoder = TemporalGraphEncoder(ppg_dim, hidden_dim, heads, dropout, weeks=weeks)
