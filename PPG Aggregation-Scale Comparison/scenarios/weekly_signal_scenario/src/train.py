...
def train_one(train, val, cfg, seed, nodes):
    ...
    return model,val_prob,epoch+1,best_auc,time.perf_counter()-started
...
def run_scale(scale, nodes, baseline, ppg, labels, folds, cfg):
    ...
    for fold in range(1,cfg["outer_folds"]+1):
        ...
        for seed in cfg["training_seeds"]:
            model,val_prob,epochs,val_auc,training_seconds=train_one(train,val,cfg,seed,nodes); threshold=youden_threshold(val.labels,val_prob)
            model.eval()
            ...
            fold_rows.append({"scale":scale,"fold":fold,"seed":seed,"validation_auc":val_auc,"epochs":epochs,"threshold":threshold,"test_n":len(test_ids),
                              "training_seconds":training_seconds,"seconds_per_epoch":training_seconds/max(epochs,1),
                              "inference_ms_per_patient":1000*inference_seconds/max(len(test_ids),1)})
